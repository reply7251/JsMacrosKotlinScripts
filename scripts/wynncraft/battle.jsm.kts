...
open class BindBoolean(key: String, value: Boolean = false, val renderName: String = key, callback: (Bind) -> Unit) : BindValue<Boolean>(key, value, callback) {
    override fun trigger() {
        value = !value
        super.trigger()
    }
...
open class WynnClass: HasBind {
    ...
    open fun onInitOverride() {
        enabled = BindBoolean("key.keyboard.f", renderName = "Enabled") {
            if(enabled.value) {
                main()
            } else {
                onDisable()
        ...
        enableMelee = BindBoolean("key.keyboard.keypad.7", true, "Melee") { updateConfig() }.bind()
    ...
    fun pressAndReleaseKeyBind(key: String) {
            ...
            while (enabled.value && World.isWorldLoaded) {
                if(!isBlocked() && !checkManual()) {
                    ...
                    if(KeyBind.pressedKeys.contains(meleeKey)) {
                        ...
                        var flag = true
                        while (flag && nextSpells.isNotEmpty()) {
    ...
    open fun updateConfig() {
        multiLineText.setText("""
            $enabled
            $spellCooldown
            Mode (${Format.coloredKey(binds["modeSelector"]?.simpleName)}): $mode <--> $quickMode
            ${Format.coloredConfig("anchor" )}Anchor ${Format.formatChar}r(${Format.coloredKey(anchor.simpleName)}): ${Format.coloredBoolean(anchor.value != Pos3D.ZERO)}
            $enableMelee
            ${maxRepeat.toString(getMaxRepeatValue())}
        """.trimIndent())
    }
    ...
    open fun waitSpell(spell: String, extraTick: Int = 0, hotbar: Int = -1, recur: Boolean = false): Boolean {
        ...
        when (spell) {
            ...
            Actions.jump -> {
                val flag = spell in KeyBind.pressedKeys
                if(flag) {
                    KeyBind.releaseKeyBind(Actions.jump)
                    Client.waitTick()
                }
                pressAndReleaseKeyBind(spell)
                if(flag) {
                    Client.waitTick()
                    KeyBind.pressKeyBind(spell)
                }
                canSkip = true
            }
            Actions.wait -> {
                ...
                if (spell.startsWith(Actions.select)) {
                    val inv = Player.openInventory()
                    spell.substring(Actions.select.length + 1).toIntOrNull()?.let {
                        inv.selectedHotbarSlotIndex = it
                    }
                } else if (spell.startsWith(Actions.prevent)) {
                    val bind = spell.split(" ").drop(1).joinToString(" ")
                    if(spell.startsWith(Actions.preventEnd)) {
                        MyKeybinding.prevents.remove(bind)
                    } else MyKeybinding.prevents.add(bind)
                    canSkip = true
                } else if (spell.startsWith(Actions.force)) {
                    val bind = spell.split(" ").drop(1).joinToString(" ")
                    if(spell.startsWith(Actions.forceEnd)) {
                        MyKeybinding.forces.remove(bind)
                    } else MyKeybinding.forces.add(bind)
                    canSkip = true
                } else {
    ...
    fun force(action: String, wait: Boolean = false, callback: () -> Unit) {
        val toCall: (String) -> Boolean = if(wait) ::waitSpell else ::addSpell
        toCall(Actions.force + " " + action)
        callback()
        toCall(Actions.forceEnd + " " + action)
    }

    fun prevent(action: String, wait: Boolean = false, callback: () -> Unit) {
        val toCall: (String) -> Boolean = if(wait) ::waitSpell else ::addSpell
...
class Warrior() : WynnClass() {
    ...
    override fun onInitOverride() {
        ...
        bloodPact = BindBoolean("key.keyboard.keypad.6", renderName = "Blood Pact") { updateConfig() }.bind()
        fly = BindBoolean("key.keyboard.keypad.4", renderName = "Fly") { updateConfig() }.bind().setPersist()
...
class Assassin() : WynnClass() {
    ...
    override fun onInitOverride() {
        ...
        slowFall = BindBoolean("key.keyboard.keypad.4", true, "Slow Fall") { updateConfig() }.bind()
        dive = BindBoolean("key.keyboard.keypad.1", true, "Dive") { updateConfig() }.bind()
    ...
    fun smoke() = waitSpell(Actions.cast4)

    fun multiHit() = waitSpell(Actions.cast3)

    fun dash() = waitSpell(Actions.cast2)
    ...
    override fun updateConfig() {
        ...
        if (mode == Mode.Acrobat) {
            multiLineText.appendInNewLine(dive.toString())
        }
    }
        ...
        thread {
            while (!terminated) {
                ...
                if(slowFall.value && enabled.value) {
                    if (Models.CharacterStats.blocksAboveGround > 1)
...
class Mage() : WynnClass() {
    ...
    override fun onInitOverride() {
        enableIceSnake = BindBoolean("key.keyboard.keypad.4", true, "IceSnake") {
            updateConfig()
        }.bind()
        enableMeteor = BindBoolean("key.keyboard.keypad.1", true, "Meteor") {
            updateConfig()
        }.bind()
...
class Archer() : WynnClass() {
    ...
    override fun onInitOverride() {
        autoEscape = BindBoolean("key.keyboard.keypad.1", renderName = "Auto Escape") {
            updateConfig()
        }.bind("escape")
    ...
    override fun chooseAction() {
        ...
        } else if(isHoldItem("spring")) {
            if(counter++ % 3 == 0) waitSpell(Actions.cast4)
            waitSpell(Actions.cast1)
            for(i in 1..getMaxRepeatValue())
                addSpell(Actions.cast3)
...
class Shaman() : WynnClass() {
    ...
    val modes = arrayOf(Mode.None, Mode.TotemSpam, Mode.Acolyte, Mode.AuraSpam, Mode.PuppetBomber, Mode.AfkAcolyte, Mode.Melee)
    ...
    override fun onInitOverride() {
        ...
        enableAwakened = BindBoolean("key.keyboard.keypad.6", renderName = "Awakened") {
            updateConfig()
        }.bind()

        fly = BindBoolean("key.keyboard.keypad.4", renderName = "Fly") {
            updateConfig()
            flyCounter = -1
        }.bind().setPersist()

        enableAura = BindBoolean("key.keyboard.keypad.3", true, "Aura") {
            updateConfig()
        }.bind()
    ...
    override fun updateConfig() {
        super.updateConfig()
        multiLineText.appendInNewLine("""
            Mask (${Format.coloredKey(binds["mask"]?.simpleName)}): $targetMask
            $enableAwakened
            $fly
            $enableAura
        """.trimIndent())
    ...
    override fun chooseAction() {
        ...
        if(mode == Mode.Acolyte) {
            if(World.time - lastTotem > 50 && (blood > 59 || !isSneaking) && (totem == null || Models.ShamanTotem.activeTotems.any { it.time !in 3..15 })) {
            ...
            if (fullSummoner) {
                ...
                if (flag1 || flag2) {
                    force(Actions.sneak) {
                        if (flag1) addSpell(Actions.cast2)
                        if (flag2) addSpell(Actions.cast3)
                    }
                }
            }
    ...
    override fun onDisable() {
        ...
        if(KeyBind.pressedKeys.contains(enabled.key) && (commander || bird)) {
            thread {
                Client.waitTick(10)
				if(!enabled.value) {
					force(Actions.sneak, true) {
						if(commander) waitSpell(Actions.cast3)
                        if(bird) waitSpell(Actions.cast2)
					}
				}
            }
        }
...
class MixinKeyBinding {
    @CInject(method = ["method_1434"], target = [CTarget("RETURN")], cancellable = true)
...
class WynnListener {
    ...
    fun worldStateChanged(event: WorldStateEvent) {
        ...
        if(event.newState == WorldState.WORLD && !characterUpdated && counter < 2) {
            thread {
                Client.waitTick(60)
                if(!characterUpdated) {
...
//            SharedLibraries.Chat.log("test: ${barProgress.value.current()}")
//            match?.group(1)?.let {
//                SharedLibraries.Chat.log(SharedLibraries.Chat.createTextBuilder().append(it).withClickEvent("copy_to_clipboard", it).build())
//            }
    }
}
