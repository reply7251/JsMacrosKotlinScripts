package me.hellrevenger.generated.Map_ConfiguredFeature
import kotlin.reflect.*
import me.hellrevenger.generated.*
/**
 * comp_333
 */
fun <FC, F>ConfiguredFeature<FC, F>.config() where FC: FeatureConfig, F: Feature<FC> = this.comp_333()
/**
 * method_12862
 */
fun <FC, F>ConfiguredFeature<FC, F>.generate(world: StructureWorldAccess?, chunkGenerator: ChunkGenerator?, random: Random?, origin: BlockPos?) where FC: FeatureConfig, F: Feature<FC> = this.method_12862(world, chunkGenerator, random, origin)
/**
 * method_30648
 */
fun <FC, F>ConfiguredFeature<FC, F>.getDecoratedFeatures() where FC: FeatureConfig, F: Feature<FC> = this.method_30648()
/**
 * comp_332
 */
fun <FC, F>ConfiguredFeature<FC, F>.feature() where FC: FeatureConfig, F: Feature<FC> = this.comp_332()
