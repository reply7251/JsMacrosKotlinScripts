package me.hellrevenger.generated.Map_StructurePoolElement
import kotlin.reflect.*
import me.hellrevenger.generated.*
object StructurePoolElementKt {
    /**
     * method_61016
     */
    fun ofProcessedSingle(id: String, processorListEntry: RegistryEntry<StructureProcessorList>, liquidSettings: StructureLiquidSettings) = StructurePoolElement.method_61016(id, processorListEntry, liquidSettings)
    /**
     * method_30434
     */
    fun ofSingle(id: String) = StructurePoolElement.method_30434(id)
    /**
     * method_30438
     */
    fun ofEmpty() = StructurePoolElement.method_30438()
    /**
     * method_30426
     */
    fun ofProcessedLegacySingle(id: String, processorListEntry: RegistryEntry<StructureProcessorList>) = StructurePoolElement.method_30426(id, processorListEntry)
    /**
     * method_30421
     */
    fun ofFeature(placedFeatureEntry: RegistryEntry<PlacedFeature>) = StructurePoolElement.method_30421(placedFeatureEntry)
    /**
     * method_30425
     */
    fun ofLegacySingle(id: String) = StructurePoolElement.method_30425(id)
    /**
     * method_30429
     */
    fun ofList(elementGetters: MutableList<function_Function<net.minecraft.class_3785.class_3786, out StructurePoolElement>>) = StructurePoolElement.method_30429(elementGetters)
}