package me.hellrevenger.generated.Map_ModifiableWorld
import kotlin.reflect.*
import me.hellrevenger.generated.*
/**
 * method_8651
 */
fun ModifiableWorld.breakBlock(pos: BlockPos, drop: Boolean, breakingEntity: Entity) = this.method_8651(pos, drop, breakingEntity)
/**
 * method_22352
 */
fun ModifiableWorld.breakBlock(pos: BlockPos, drop: Boolean) = this.method_22352(pos, drop)
/**
 * method_30093
 */
fun ModifiableWorld.breakBlock(pos: BlockPos, drop: Boolean, breakingEntity: Entity, maxUpdateDepth: Int) = this.method_30093(pos, drop, breakingEntity, maxUpdateDepth)
/**
 * method_8650
 */
fun ModifiableWorld.removeBlock(pos: BlockPos, move: Boolean) = this.method_8650(pos, move)
/**
 * method_8649
 */
fun ModifiableWorld.spawnEntity(entity: Entity) = this.method_8649(entity)
/**
 * method_30092
 */
fun ModifiableWorld.setBlockState(pos: BlockPos, state: BlockState, flags: Int, maxUpdateDepth: Int) = this.method_30092(pos, state, flags, maxUpdateDepth)
/**
 * method_8652
 */
fun ModifiableWorld.setBlockState(pos: BlockPos, state: BlockState, flags: Int) = this.method_8652(pos, state, flags)
