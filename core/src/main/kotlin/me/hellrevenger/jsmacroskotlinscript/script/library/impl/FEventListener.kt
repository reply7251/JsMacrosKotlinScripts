...
class FEventListener(val context: BaseScriptContext<*>) : PerExecLibrary(context) {
    ...
    fun <T: Any> registerForScript(event: net.fabricmc.fabric.api.event.Event<T>, listener: T) {
        (event as IEvent<T>).registerForScript(listener)
        (ctx as? KotlinScriptContext)?.onContextClosed {
            event.unregisterForScript(listener)
        }
    }

    @Suppress("unchecked_cast")
    fun <T: Any> unregisterForScript(event: net.fabricmc.fabric.api.event.Event<T>, listener: T) {
        (event as IEvent<T>).unregisterForScript(listener)
    }
}
...
class Listener<T: BaseEvent>(val context: BaseScriptContext<*>, private val eventName: String, private val callback: (T) -> Unit, private val joined: Boolean) :
    IEventListener {
    init {
